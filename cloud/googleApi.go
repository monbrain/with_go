...
func getConfigPath(folder, nick, authType string) string {
...
// func getService(api, authType string) *http.Client {
func ApiClient(api_name, bot_nick, user_nick string) *http.Client {
	// I had to use a Service Account to build my app
	// folder := "/home/ubuntu/dev/inGo/with_go/on_cloud/"
	folder := "/home/ubuntu/dev/inGo/with_go/settings/" // TODO: .env or 시스템 환경 설정
	...
	path := getConfigPath(folder, nick, authType)
